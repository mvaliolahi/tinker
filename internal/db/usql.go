...
// Connect opens an interactive database session using the best available CLI.
// With native drivers compiled in, interactive sessions are the only reason
// to shell out to external CLIs.
...
// Exec runs a query via an external CLI tool (fallback when no native connection).
// Prefer using native database/sql queries via the session's methods instead.
...
// ExecFormatted runs a query with tabular formatting via an external CLI tool
// (fallback when no native connection).
// Prefer using native database/sql queries via the session's methods instead.

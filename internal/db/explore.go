...
func (s *Session) ExploreTable(table string, limit int) ([]string, [][]string, error) {
        ...
        q := fmt.Sprintf("SELECT * FROM %s LIMIT %d", quoteIdent(table), limit)
...
func readRows(rows interface{ Columns() ([]string, error); Next() bool; Scan(...interface{}) error }) ([]string, [][]string, error) {

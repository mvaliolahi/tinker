...
func Generate(result *detect.Result, dir string) error {
	...
	return os.WriteFile(filepath.Join(dir, "tinker.toml"), []byte(sb.String()), 0600)
}
...
func writeEnvTemplate(sb *strings.Builder, _ *detect.DockerResult) {
...
func SaveLogConfig(files []string, dir string) error {
	...
	if logStart >= 0 {
		...
		return os.WriteFile(tomlPath, []byte(updated), 0600)
	}

...
var all = []Dep{
	...
	// Note: These are optional for interactive sessions only — native Go drivers
	// handle all one-shot db commands (tables, describe, count, find, etc.)
...
func pipInstall(dep Dep) error {
	...
	if _, err := exec.LookPath("pipx"); err == nil {
		cmd := exec.Command("pipx", "install", dep.Pip) //nolint:gosec // dep.Pip is from our own config
	...
	cmd := exec.Command(pipCmd, "install", "--break-system-packages", "--user", dep.Pip) //nolint:gosec // dep.Pip is from our own config
	out, err := cmd.CombinedOutput()
	if err == nil {
		return nil
	}
	...
	cmd2 := exec.Command(pipCmd, "install", "--user", dep.Pip) //nolint:gosec // dep.Pip is from our own config
...
func cloneAndBuild(dep Dep) error {
	...
	cmd := exec.Command("go", "install", installPath+"@latest") //nolint:gosec // installPath from our own config
	out, err := cmd.CombinedOutput()
	if err == nil {
		return nil
	}
